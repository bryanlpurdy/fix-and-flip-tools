...
- **Deal Analyzer desktop footer** — `.da-footer` is a fixed bottom bar (`left: var(--da-sidebar-w); right: 0`) containing Expected Profit (left) and Save Deal + Export PDF buttons (right). On desktop, `header .btn-save` and `header .btn-pdf` are hidden with `display: none !important` — scoped to `header` to avoid hiding the footer buttons. Both `.inputs-panel` and `.results-panel` have `padding-bottom: 80px` inside `@media (min-width: 1025px)` so content doesn't run into the fixed footer.
- **Deal Analyzer `+ New` button** — lives in the sidebar header (`.da-sidebar-hdr`) on desktop, not in the main header.
- **Deal Analyzer mobile delete button** — `.mdc-del` is styled as a red-bordered "Delete" text button (`border: 1px solid var(--danger); color: var(--danger); border-radius: 4px`). Previous version was a subtle `✕` icon with muted color — updated to match the consistent Walkthrough / Net Sheet pattern.
